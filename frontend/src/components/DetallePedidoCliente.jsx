...
function DetallePedidoCliente({ pedidoId }) {
  ...
  const fetchPedidoData = async () => {
    setLoading(true);
    try {
      const response = await publicAxios.get(`/pedidos/cliente/tracking/${pedidoId}`);
  ...
  // Extraer datos del repartidor (puede estar en la segunda posición del array de Usuarios)
  const repartidor = pedidoData.Usuarios && pedidoData.Usuarios.length > 1 
    ? pedidoData.Usuarios[1] 
    : (pedidoData.Usuarios && pedidoData.Usuarios[0] ? pedidoData.Usuarios[0] : null);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto bg-white rounded-lg shadow-md p-4 sm:p-6 md:p-8">
      ...
      {repartidor && (repartidor.nombre || repartidor.telefono) && (
